...
;(function ($) {
    ...
    $.fn.metroPivot = function (settings) {
        ...
        $.extend(this,{
			...
			addNewHeader : function(header) {
				if(this.animating) return;

				if ( this.headers.children(":contains('"+header+"')") )
				{
					this.headers.children(":contains('"+header+"')").remove();
				}

				var pivot = this;

				var headers = pivot.headers;
                var items = pivot.items;
							
                var headerItem = pivot.headerItemTemplate().html(header).fadeTo(0, pivot.headerOpacity);
                var pivotItem = pivot.pivotItemTemplate();
				pivotItem.attr("id", "users_display_all");


				var sz = headers.size();

                headerItem.attr("index", 3 );
                headerItem.click(function() { pivot.pivotHeader_Click($(this)); });
				
				headers.append(headerItem);
				items.append(pivotItem);

				this.goToItemByIndex( 3 );
            ...
            pivotHeader_Click : function (me) {
                ...
                this.animating = true;
				 ...
				 var curr = this.headers.children(".header.current");

				if( curr  )
				{
					if( curr.length )
					{
						if( this.headers.children(".header.current")[0].innerHTML.indexOf("Kullanıcılar") != -1 )
						{
							this.headers.children(":contains('Kullanıcılar')").remove();
							this.items.children(".pivotItem:nth(3)").remove();
							$("#users_display_all").empty();
						}
					}
				}


				if( me.context.innerHTML.indexOf("PC") != -1 )
				{
					$("#pc_display_all").load("pc_display_all.aspx");
				}
				else if( me.context.innerHTML.indexOf("Ofis Aygıtları") != -1 )
				{
					$("#office_display_all").load("office_display_all.aspx");
				}
				else if( me.context.innerHTML.indexOf("Yazılım") != -1 )
				{
					$("#software_display_all").load("software_display_all.aspx");
				}
				if( me.context.innerHTML.indexOf("Kullanıcılar") != -1 )
				{
					$("#users_display_all").load("users.aspx");
				}
